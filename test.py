...
st.markdown(
    ...
    .card{
        width:230px;padding:1.7rem 1rem;background:#ffffff;
        border:2px dotted #3f51b5;border-radius:20px;text-align:center;
        transition:transform .15s,box-shadow .15s;
        cursor:pointer;
    }
    .card:hover{
        transform:translateY(-6px);
        box-shadow:0 8px 16px rgba(63,81,181,.25);
    }
    .card h3{
        font-family:'Chewy',cursive;font-size:1.8rem;margin:0;color:#263238;
    }
    .card p{
        margin:.45rem 0 0;
        font-size:.9rem;font-style:italic;color:#607d8b;
    }

    /* gradient button */
    .stButton>button{
        background:linear-gradient(135deg,#3f51b5 0%,#009688 100%);
        color:#fff;border:none;padding:.8rem 1.8rem;border-radius:.7rem;
...
<div class="landing-wrapper">
  ...
  <p class="tagline">
    A platform that <strong>gamifies open problems in theoretical fields</strong> —
    turning cutting-edge research questions into interactive coding challenges.
  </p>
...
st.sidebar.write(f"**Email:** {st.session_state['user_info']['email]()_]()_
