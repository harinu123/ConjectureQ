...
# ─────────────────────────────────────────────────────────────────────────────
#  Global CSS  (Bubble font, pink palette, card buttons, etc.)
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(
    """
    <style>
      ...
      .tagline {
          font-size: 1.25rem; letter-spacing: .5px; color: #444;
      }

      /* cards for “compete / sponsor” */
      .card-row   {display: flex; gap: 2rem; margin-top: 2.5rem;}
      .card {
          width: 220px; padding: 1.8rem 1rem;
          text-align: center; border: 2px dotted #ff6ec7;
          border-radius: 18px; background: #fff;
          transition: transform .15s ease, box-shadow .15s ease;
      }
      .card:hover {
          transform: translateY(-6px);
          box-shadow: 0 6px 12px #ff9ae540;
      }
      .card h3 {margin: 0; font-family: 'Chewy', cursive; font-size: 1.6rem; color:#333;}
      .card p  {margin: .35rem 0 0; font-size: .85rem; font-style: italic; color:#777;}

      /* gradient button (mobile fallback) */
      .stButton > button {
          background: linear-gradient(135deg,#7f00ff 0%,#e100ff 100%);
          color:#fff; border:none; padding:.8rem 1.7rem;
          border-radius:.7rem; font-weight:600; font-size:1.05rem;
      }
    </style>
    """,
    unsafe_allow_html=True,
...
if not st.session_state.show_app:
    ...
    st.markdown(
        textwrap.dedent(
            """
            <div class="landing-wrapper">
              <img src="https://yourcdn.com/brain_logo.png" class="brand-logo" alt="logo">
              <h1 class="brand-name">brainrotcode</h1>
              <p class="tagline">launch your AI bots into live battles</p>

              <div class="card-row">
                <div class="card" onclick="window.location.hash='#compete'">
                  <h3>compete</h3>
                  <p>for developers</p>
                </div>
                <div class="card" onclick="window.location.hash='#sponsor'">
                  <h3>sponsor</h3>
                  <p>for companies</p>
                </div>
              </div>
    ...
    # fallback Enter button for keyboard nav / small screens
    st.write("")
...
```"""
    ...
    if st.button("Submit Solver"):
        out = backend.run_solution_and_get_results(st.session_state["user_info"]["email"], code)
        st.json(out)
...
with tabs[4]:
    ...
    test_input = st.text_area("Paste list of 784-length rows")
    if st.button("Submit Batch"):
        fb = backend.run_tester_and_get_feedback(st.session_state["user_info"]["email"], test_input)
        st.json(fb)
