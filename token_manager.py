...
class AuthTokenManager:
    ...
    def set_token(self, email: str, name: str, picture: str):
        exp_date = (
            datetime.now() + timedelta(days=self.token_duration_days)
        ).timestamp()
        token = self._encode_token(email, name, picture, exp_date)
    ...
    def _encode_token(self, email: str, name: str, picture: str, exp_date: float):
        encoded = jwt.encode(
            {"email": email, "name": name, "picture": picture, "exp": exp_date},
            self.token_key,
            algorithm="HS256",
        )
        return encoded
